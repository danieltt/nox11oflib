...
//	int process_message(void* msg, int size);
//	int process_fns(void* msg);
...
//	void process_rule_reverse(EPoint* rule, Flow *flow, uint32_t buf);
//	int install_rule(uint64_t id, int p_in, int p_out, int buf);
	int install_rule(uint64_t id, int p_in, int p_out, Flow* flow, int buf);
	int remove_rule(FNSRule rule);
//	int install_fns(fnsDesc* fns);
